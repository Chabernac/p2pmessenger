...
import chabernac.space.DefaultMoveableShape;
...
import chabernac.space.TranslateManager;
import chabernac.space.TranslateManagerContainer;
import chabernac.space.World;
import chabernac.space.buffer.SBuffer;
...
public class HollowWorld extends DefaultExitFrame{
	...
	private void startEngineFullScreen(){
		setVisible(false);
		Frame3D theFrame = new Frame3D(buildWorld(), myCamera, null);
...
//			theWorld.addShape(theCube3);
	...
	private void makeRotatingShape(World aWorld) throws PolygonException, MatrixException{
	  	DefaultMoveableShape theShape = new DefaultMoveableShape(4);
		ShapeFactory.makeCube(theShape, new Point3D(200,400,500),200,400,300);
		theShape.done();
