...
public class IPAddress {
	...
	public static IPAddress getLocalIPAddress() throws InvalidIpAddressException {
		try {
			...
			int theNetworkPrefixLength = networkInterface.getInterfaceAddresses().get(0).getNetworkPrefixLength();
			// bug in java SDK? sometimes -1 is returned as network prefix length
			if (theNetworkPrefixLength == -1)
				theNetworkPrefixLength = 24;
			return new IPAddress(localHost.getHostAddress() + "/" + theNetworkPrefixLength);
		} catch (Exception e) {
	...
	private void createParts(String aString) throws InvalidIpAddressException {
		...
		if (!theMatcher.matches())
			throw new InvalidIpAddressException("This is not a valid ip address '" + aString + "'");

		for (int i = 0; i < 4; i++) {
			int thePart = Integer.parseInt(theMatcher.group(i + 1));
			if (thePart < 0)
				throw new InvalidIpAddressException("Number can not be < 0");
			if (thePart > 255)
				throw new InvalidIpAddressException("Number can not be > 255");
			myParts[i] = thePart;
		}
	...
	public boolean isPrivate() {
		if (myParts[0] == 10)
			return true;
		if (myParts[0] == 192 && myParts[1] == 168)
			return true;
		if (myParts[0] == 172 && myParts[1] >= 16 && myParts[1] <= 31)
			return true;
		return false;
	}

	public IPClass getIPClass() {
		if (myParts[0] >= 0 && myParts[0] <= 127)
			return IPClass.A;
		if (myParts[0] >= 128 && myParts[0] <= 191)
			return IPClass.B;
		if (myParts[0] >= 192 && myParts[0] <= 223)
			return IPClass.C;
	...
	private void determineNetworkPrefixLength() {
		if (myNetworkPrefixLength == -1) {
			...
			if (theClass == IPClass.A)
				myNetworkPrefixLength = 8;
			else if (theClass == IPClass.B)
				myNetworkPrefixLength = 16;
			else if (theClass == IPClass.C)
				myNetworkPrefixLength = 24;
		}
	}
