...
public class NetTools {
	...
	private static final Map<String, SimpleNetworkInterface> myHostNetworkInterface = new HashMap<String, SimpleNetworkInterface>();
	...
	public static synchronized SimpleNetworkInterface getNetworkInterfaceForLocalIP(String aLocalIp) throws SocketException{
		if(!myHostNetworkInterface.containsKey(aLocalIp)){
			Enumeration<NetworkInterface> theInterfaces = NetworkInterface.getNetworkInterfaces();
			while(theInterfaces.hasMoreElements()){
				NetworkInterface theInterface = theInterfaces.nextElement();
				for(InterfaceAddress theAddress : theInterface.getInterfaceAddresses()){
					if(aLocalIp.equalsIgnoreCase( theAddress.getAddress().getHostAddress() )){
						myHostNetworkInterface.put(aLocalIp, new SimpleNetworkInterface(theInterface.getDisplayName(), theInterface.isLoopback(), theInterface.getHardwareAddress(), aLocalIp));
					}
				}
			}
		}
