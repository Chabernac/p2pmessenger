...
public class Matrix extends DefaultEasterEggPaintable {
	...
	private ArrayList myMessages = null;
	...
	public Matrix(ChatMediator aMediator){
		super(aMediator);
		myRandom = new Random();
	...
	private void init(){
		...
		try{
			ArrayList theMessages = myMediator.getMessageArchive().getAllMessages();
			TEXTS = new String[5 + theMessages.size() + myMessages.size()];
			int j = 0;
			TEXTS[j++] = "this software is cool";
			TEXTS[j++] = myMediator.getMessengerClientService().getUser().getFirstName();
			TEXTS[j++] = myMediator.getMessengerClientService().getUser().getLastName();
			TEXTS[j++] = myMediator.getMessengerClientService().getUser().getUserName();
			TEXTS[j++] = myMediator.getMessengerClientService().getUser().getVersion();
			for(int i=0;i<theMessages.size();i++){
				TEXTS[j++] = ((Message)theMessages.get(i)).getMessage();
			}
			for(Iterator i=myMessages.iterator();i.hasNext();){
				TEXTS[j++] = (String)i.next();
			}
